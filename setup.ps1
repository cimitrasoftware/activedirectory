# Setup Cimitra Active Directory Practice Cimitra Apps & Scripts
...
if($Debug){
Write-Output "Cimitra Agent Config File: ${CIMITRA_AGENT_CONFIG_FILE}"
Start-Sleep 2
}
...
$response = Invoke-WebRequest -SkipCertificateCheck -Uri "${URL}" `
-Method "POST" `
-Headers @{
...
$theResponse = Invoke-WebRequest -SkipCertificateCheck -Uri "${URL}" `
-Method "POST" `
-Headers @{
"Authorization"="Bearer $token"
} `
...
$response = Invoke-RestMethod -SkipCertificateCheck -Uri $uri/users/login -Method POST -Body $payload -ContentType "application/json"
...
Write-Output "Invoke-RestMethod -SkipCertificateCheck -Uri $uri/agent/$cimitraAgentId -Method GET -Headers $headers -UseBasicParsing"
}

try{
Invoke-RestMethod -SkipCertificateCheck -Uri $uri/agent/$cimitraAgentId -Method GET -Headers $headers -UseBasicParsing | fl > $TEMP_FILE_ONE
}
catch{
Remove-Item -Path $TEMP_FILE_ONE -Force 2>&1 | out-null
...
Write-Output "Looking for Agent: $cimitraAgentId"
...
$b = Get-Content -Path $TEMP_FILE_ONE
@(ForEach ($a in $b) {$a.Replace(' ', '')}) > $TEMP_FILE_TWO



$agentDetail = Get-Content "$TEMP_FILE_TWO" | Select-String -Pattern "$cimitraAgentId" -CaseSensitive -SimpleMatch

$agentName = Get-Content "$TEMP_FILE_TWO" | Select-String -Pattern "name:" -CaseSensitive -SimpleMatch

if($Debug){
Write-Output "TEMP FILE TWO"
Write-Output "$TEMP_FILE_TWO"
Write-Output "Agent Detail: $agentDetail"
Write-Output "Agent Name Raw: $agentName"
Start-Sleep -s 5
}

$agentName = Write-Output "$agentName" | %{ $_.Split(':')[1]; }
...
Invoke-RestMethod -SkipCertificateCheck -Uri $uri/apps -Method GET -Headers $headers -UseBasicParsing > $TEMP_FILE_ONE
...
Invoke-RestMethod -SkipCertificateCheck -Uri $uri/apps/$adRootFolderId/children -Method GET -Headers $headers -UseBasicParsing > $TEMP_FILE_ONE
...
Invoke-RestMethod -SkipCertificateCheck -Uri $uri/apps/$adFolderId/children -Method GET -Headers $headers -UseBasicParsing > $TEMP_FILE_ONE
...
Invoke-RestMethod -SkipCertificateCheck -Uri $uri/apps/$adUserFolderId/children -Method GET -Headers $headers -UseBasicParsing > $TEMP_FILE_ONE
...
# Get the ACTIVE DIRECTORY | DELGATE | USER MANAGEMENT | GROUPS folder


$CONFIRM_GROUPS_FOLDER=(ConfirmFromConfigFile "${PSScriptRoot}\settings.cfg" "ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL")

if ( $CONFIRM_GROUPS_FOLDER ) 
{ 
Set-Variable -Name ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL -Value $CONFIG$ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL }
else{
Set-Variable -Name ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL -Value 'GROUPS' 
}


if ((Get-Content "$TEMP_FILE_ONE" | Select-String -CaseSensitive "\b${ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL}\b" )){

$adUserGroupsIdOne = (Get-Content $TEMP_FILE_ONE | Select-String -SimpleMatch -CaseSensitive "${ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL}" -Context 1 | Select-Object -First 1 ) 

$adUserGroupsIdTwo = ($adUserGroupsIdOne -split '\n')[0]

$adUserGroupsIdThree = ( $adUserGroupsIdTwo | %{ $_.Split(':')[1];} )

$adUserGroupsId = $adUserGroupsIdThree.Trim()

$global:adUserGroupsId = $adUserGroupsId

}else{
Write-Output "Error: Cannot Create or Discover the Folder: ${ACTIVE_DIRECTORY_DELEGATED_FOLDER_LABEL} | ${ACTIVE_DIRECTORY_USER_MANAGEMENT_FOLDER_LABEL} | ${ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL}"
Remove-Item -Path $TEMP_FILE_ONE -Force
exit 1
}
...
Invoke-RestMethod -SkipCertificateCheck -Uri https://$uri/api/apps/$adUserFolderId/children -Method GET -Headers $headers -UseBasicParsing > $TEMP_FILE_ONE
...
Invoke-RestMethod -SkipCertificateCheck -Uri $uri/apps/$delegateFolderId/children -Method GET -Headers $headers -UseBasicParsing > $TEMP_FILE_ONE
...
Invoke-RestMethod -SkipCertificateCheck -Uri $uri/apps/$userManagementFolderId/children -Method GET -Headers $headers -UseBasicParsing > $TEMP_FILE_ONE
...
# Confirm/Create the ACTIVE DIRECTORY | DELEGATE | USER MANAGEMENT | GROUPS folder

$CONFIRM_USER_GROUPS_FOLDER=(ConfirmFromConfigFile "${PSScriptRoot}\settings.cfg" "ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL")

if ( $CONFIRM_USER_GROUPS_FOLDER ) 
{ 
Set-Variable -Name ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL -Value $CONFIG$ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL }
else{
Set-Variable -Name ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL -Value 'GROUPS' 
...
if ((Get-Content "$TEMP_FILE_ONE") -match ": ${ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL}"){
Write-Output ""
Write-Output "${ACTIVE_DIRECTORY_DELEGATED_FOLDER_LABEL} | ${ACTIVE_DIRECTORY_USER_MANAGEMENT_FOLDER_LABEL} | ${ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL} - Folder Exists"
Write-Output ""
}else{
Write-Output ""
Write-Output "${ACTIVE_DIRECTORY_DELEGATED_FOLDER_LABEL} | ${ACTIVE_DIRECTORY_USER_MANAGEMENT_FOLDER_LABEL} | ${ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL} - Folder Create"
Write-Output ""
CREATE_CIMITRA_FOLDER_ENTITY "${ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL}" "Cimitra Active Directory Integration ${ACTIVE_DIRECTORY_DELEGATED_FOLDER_LABEL} | ${ACTIVE_DIRECTORY_USER_MANAGEMENT_FOLDER_LABEL} | ${ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL} App Delegation Folder." "$userManagementFolderId"
}
...
confirmConfigSetting "$settingsFile" "ACTIVE_DIRECTORY_USER_GROUPS_FOLDER_LABEL" "GROUPS"
...
$jsonFile = "{`"type`":1,`"status`":`"active`",`"platform`":`"win32`",`"injectParams`":[{`"param`":`"-FirstNameIn `",`"value`":`"`",`"label`":`"First Name`",`"regex`":`"/^[0-9A-Za-z_+-= ]+`$/`",`"placeholder`":`"Jane`"},{`"param`":`"-LastNameIn `",`"value`":`"`",`"label`":`"Last Name`",`"regex`":`"/^[0-9A-Za-z_+-= ]+`$/`",`"placeholder`":`"Doe`"}],`"interpreter`":`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`",`"command`":`"${scriptRoot}\\SearchForUser.ps1`",`"params`":`" -ShowErrors `",`"agentId`":`"${cimitraAgentId}`",`"name`":`"SEARCH ENTIRE TREE FOR USER`",`"notes`":`"`",`"description`":`"Search the Entire Active Directory Tree for a User.`",`"parentFolderId`":`"${adUserChangesId}`"}"
CREATE_CIMITRA_APP "SEARCH ENTIRE TREE FOR USER" "SearchForUser.ps1" "${adUserChangesId}" "${adExcludeFolderId}" "$jsonFile"
...
# GROUP Folder                #
###############################
$jsonFile = "{`"type`":1,`"status`":`"active`",`"platform`":`"win32`",`"injectParams`":[{`"EncapsulateChar`":`"'`",`"param`":`"-GroupNameIn `",`"value`":`"`",`"label`":`"GROUP NAME`",`"regex`":`"/^[0-9A-Za-z_+-= ]+`$/`",`"placeholder`":`"ACCOUNTING GROUP`"}],`"interpreter`":`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`",`"command`":`"${scriptRoot}\\GetGroupInfo.ps1`",`"params`":`" -ShowErrors `",`"agentId`":`"${cimitraAgentId}`",`"name`":`"GET GROUP INFO`",`"notes`":`"`",`"description`":`"Get Information About an Active Directory Group.`",`"parentFolderId`":`"${adUserGroupsId}`"}"
CREATE_CIMITRA_APP "GET GROUP INFO" "GetGroupInfo.ps1" "${adUserGroupsId}" "${adExcludeFolderId}" "$jsonFile"
...
$jsonFile = "{`"type`":1,`"status`":`"active`",`"platform`":`"win32`",`"injectParams`":[{`"param`":`"-ComputerNameIn `",`"value`":`"`",`"label`":`"Current Computer Name`",`"regex`":`"/^[0-9A-Za-z_+-=]+`$/`",`"placeholder`":`"WIN-COMPUTER-ONE`"},{`"param`":`"-NewComputerNameIn `",`"value`":`"`",`"label`":`"New Computer Name`",`"regex`":`"/^[0-9A-Za-z_+-=]+`$/`",`"placeholder`":`"WIN_COMPUTER_TWO`"}],`"interpreter`":`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`",`"command`":`"${scriptRoot}\\RenameComputer.ps1`",`"params`":`" -ShowErrors `",`"agentId`":`"${cimitraAgentId}`",`"name`":`"RENAME COMPUTER`",`"notes`":`"NOTE - Make sure that in the file 'settings.cfg' the 'AD_COMPUTER_CONTEXT' value is properly configured with the Active Directory Context where this script is supposed to be looking to.`",`"description`":`"Rename a Computer Object in Active Directory`",`"parentFolderId`":`"${adComputerFolderId}`"}"
...
$jsonFile = "{`"type`":1,`"status`":`"active`",`"platform`":`"win32`",`"injectParams`":[{`"param`":`"-ComputerNameIn `",`"value`":`"`",`"label`":`"Computer Name`",`"regex`":`"/^[0-9A-Za-z_+-=]+`$/`",`"placeholder`":`"WIN-COMPUTER-ONE`"}],`"interpreter`":`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`",`"command`":`"${scriptRoot}\\RemoveComputer.ps1`",`"params`":`" -ShowErrors `",`"agentId`":`"${cimitraAgentId}`",`"name`":`"REMOVE COMPUTER`",`"notes`":`"NOTE - Make sure that in the file 'settings.cfg' the 'AD_COMPUTER_CONTEXT' value is properly configured with the Active Directory Context where this script is supposed to be looking to.`",`"description`":`"Remove a Computer From Active Directory`",`"parentFolderId`":`"${adComputerFolderId}`"}"
CREATE_CIMITRA_APP "REMOVE COMPUTER" "RemoveComputer.ps1" "${adComputerFolderId}" "${adExcludeFolderId}" "$jsonFile"

$jsonFile = "{`"type`":1,`"status`":`"active`",`"platform`":`"win32`",`"injectParams`":[{`"param`":`"-ComputerNameIn `",`"value`":`"`",`"label`":`"Computer Name`",`"regex`":`"/^[0-9A-Za-z_+-=]+`$/`",`"placeholder`":`"WIN-COMPUTER-ONE`"}],`"interpreter`":`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`",`"command`":`"${scriptRoot}\\SearchForComputer.ps1`",`"params`":`" -ShowErrors `",`"agentId`":`"${cimitraAgentId}`",`"name`":`"SEARCH FOR COMPUTER`",`"notes`":`"`",`"description`":`"Search The Entire Active Directory Tree For a Computer`",`"parentFolderId`":`"${adComputerFolderId}`"}"
CREATE_CIMITRA_APP "SEARCH FOR COMPUTER" "SearchForComputer.ps1" "${adComputerFolderId}" "${adExcludeFolderId}" "$jsonFile"
