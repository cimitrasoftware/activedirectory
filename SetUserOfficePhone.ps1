...
$lastNameIn = ([Regex]'(?is)(?:(?<=\-lastNameIn).+(?=-officePhoneIn))').Match(($args -join "`n")).Value 
...
Write-Host ""
Write-Host "Set a User's Office Phone in Active Directory"
Write-Host ""
Write-Host "[ HELP ]"
Write-Host ""
...
Write-Host ""
Write-Host ".\$scriptName -firstNameIn <user first name> -lastNameIn <user last name> -officePhoneIn <office phone number> -contextIn <Active Directory context (optional if specified in settings.cfg)>"
Write-Host ""
Write-Host "-OR-"
Write-Host ""
Write-Host ".\$scriptName -setContext <Active Directory context (optional if specified in settings.cfg)> -firstNameIn <user first name> -lastNameIn <user last name> -officePhoneIn <office phone number> "
Write-Host ""
Write-Host "[ EXAMPLES ]"
Write-Host ""
...
Write-Host "-OR-"
Write-Host ""
Write-Host "Example: .\$scriptName -setContext OU=USERS,OU=DEMO,OU=CIMITRA,DC=cimitrademo,DC=com -firstNameIn Jane -lastNameIn Doe -officePhoneIn 801-555-1212"
Write-Host ""
Write-Host "-OR-"
Write-Host ""
...
# Modify the user
try{
Set-ADUser -Identity "CN=${firstNameIn} ${lastNameIn},$context" -OfficePhone "$officePhoneIn"
}catch{
