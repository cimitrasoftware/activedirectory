...
$lastNameIn = ([Regex]'(?is)(?:(?<=\-lastNameIn).+(?=-departmentIn))').Match(($args -join "`n")).Value 
...
if($departmentIn.Length -gt 2){
$departmentInSet = $true
}
...
Write-Host ""
Write-Host ".\$scriptName -firstNameIn <user first name> -lastNameIn <user last name> -departmentIn <department> -contextIn <Active Directory context (optional if specified in settings.cfg)>"
Write-Host ""
Write-Host "-OR-"
Write-Host ""
Write-Host ".\$scriptName -setContext <Active Directory context (optional if specified in settings.cfg)> -firstNameIn <user first name> -lastNameIn <user last name> -departmentIn <department> "
Write-Host ""
Write-Host "[ EXAMPLES ]"
Write-Host ""
Write-Host "Example: .\$scriptName -firstNameIn Jane -lastNameIn Doe -departmentIn Development"
Write-Host ""
Write-Host "-OR-"
Write-Host ""
Write-Host "Example: .\$scriptName -setContext OU=USERS,OU=DEMO,OU=CIMITRA,DC=cimitrademo,DC=com -firstNameIn Jane -lastNameIn Doe -departmentIn Development"
Write-Host ""
Write-Host "-OR-"
Write-Host ""
Write-Host "Example: .\$scriptName -firstNameIn Jane -lastNameIn Doe -departmentIn Development -contextIn OU=USERS,OU=DEMO,OU=CIMITRA,DC=cimitrademo,DC=com"
Write-Host ""
Write-Host "[ ERROR HANDLING ]"
Write-Host ""
Write-Host "-showErrors = Show Error Messages"
Write-Host ""
Write-Host "Example: .\$scriptName -showErrors -firstNameIn Jane -lastNameIn Doe -departmentIn Development"
Write-Host ""
exit 0
}
...
if (!( $firstNameInSet -and $lastNameInSet -and $departmentInSet )){ 
ShowHelp
 }
# -------------------------------------------------
...
# Modify the user
try{
Set-ADUser -Identity "CN=${firstNameIn} ${lastNameIn},$context" -Department "$departmentIn"
}catch{
...
{
Write-Output ""
Write-Output "User: ${firstNameIn} ${lastNameIn} department changed to: ${departmentIn} : in Active Directory"
Write-Output ""
}else{
Write-Output ""
Write-Output "User: ${firstNameIn} ${lastNameIn} department phone NOT changed in Active Directory"
Write-Output ""
    if ($verboseOutputSet){
