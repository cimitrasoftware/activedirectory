...
$ExchangeUserInEmpty = [string]::IsNullOrWhiteSpace($ExchangeUserIn)
...
$global:samAccountName = "$SamAccountNameIn"
    ...
    if(!($ExchangeUserInEmpty)){
    $global:samAccountNameInSet = $true
    $global:samAccountName = "$ExchangeUserIn"
    }
...
function PositivelyIdentifyUser()
{
if(!($samAccountNameInSet)){
    ...
    if($UserDoesNotExist){
        Write-Output ""
        Write-Output "A User With The Userid: $samAccountName Does Not Exists"
        Write-Output ""
        exit 
    }else{
...
$username = "$CimitraAgentLoginAccountIn"
