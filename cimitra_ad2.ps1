...
Write-Output ""
Write-Output "To Modify a User in Exchange, Specify The Parameter -PrimarySmtpAddressIn"
Write-Output ""
Write-Output "Example: -PrimarySmtpAddressIn 'jdoe@example.com'"
Write-Output ""
return
}
...
$GiveName = $NewFirstNameIn
}else{
$GiveName = $FirstNameIn
}
...
try{
Set-User -Identity "$TheUserPrincipleName" -FirstName "$GiveName" -LastName "$SurName" -DisplayName "$GiveName $SurName" -Name "$GiveName $SurName" -ErrorAction Stop
}catch{
$err = "$_"

Write-Output ""
Write-Output "User: ${userFirstName} ${userLastName} | Not Modified In Active Directory"
Write-Output ""
    if ($verboseOutputSet){
...
Write-Output ""
Write-Output "User: ${userFirstName} ${userLastName} | Changed In Active Directory To | User: $GiveName $SurName"
...
try{
Set-Mailbox -Identity "$TheUserPrincipalName" -EmailAddressPolicyEnabled $False -WindowsEmailAddress "$TheUserPrincipalName" -PrimarySmtpAddress "$PrimarySmtpAddressIn"
}catch{
$err = "$_"
...
try{
Set-RemoteMailbox -Identity "$TheUserPrincipalName" -EmailAddressPolicyEnabled $False -WindowsEmailAddress "$TheUserPrincipalName" -PrimarySmtpAddress "$PrimarySmtpAddressIn"
}catch{
$err = "$_"
...
Write-Output ""
Write-Output "User: ${userFirstName} ${userLastName} | Changed In Exchange To | User: $GiveName $SurName | Email: $PrimarySmtpAddressIn"
...
if($AddToExchange -or $ExchangeRemoteMailbox -or $ExchangeOnSiteMailbox){
try{
