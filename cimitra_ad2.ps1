...
# Modify the user
try{
Get-ADUser -Filter * | select Name,Givenname,Surname,sAMAccountName,distinguishedName | fl
}catch{
$runResult = $true
$err = "$_"
}
...
try{$theArray = $ArrayOfGroupGUIDs.GetUpperBound(0)}catch{
...
$TheGroupName =  $TheGroupName.Name
 ...
 try{Remove-ADGroupMember -Identity $GroupGuid -Member "$samAccountName" -Confirm:$false *> $null
    }catch{
...
try{
$theManager=Get-ADUser -properties manager -Identity "$samAccountName" | select manager -ExpandProperty manager 
}catch{}
...
try{
$TheUser = Get-ADUser -Identity "$TheSamAccountName" 2> $null
}catch{
$UserDoesNotExist = $true
}
...
try{
$TheGroupName = Get-ADGroup -Identity "$ExcludeGroupGUIDIn" 
}catch{
$err = "$_"
...
# Modify the user
try{
Set-ADAccountPassword -Identity "$samAccountName" -Reset -NewPassword (ConvertTo-SecureString -AsPlainText "${TheUserPasswordIn}" -Force)
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
# Modify the user
try{
Set-ADAccountPassword -Identity "$samAccountName" -Reset -NewPassword (ConvertTo-SecureString -AsPlainText "${TheUserPassword}" -Force)
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
# Modify the user
try{
Get-ADUser -Identity "$samAccountName"  | Rename-ADObject -NewName "${NewFirstNameIn} ${userLastName}" 
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Get-ADUser -Identity "$samAccountName"  | Set-ADUser -DisplayName "${NewFirstNameIn} ${userLastName}" 
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
# Modify the user
try{
Get-ADUser -Identity "$samAccountName"  | Rename-ADObject -NewName "${userFirstName} ${newLastNameIn}" 
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Get-ADUser -Identity "$samAccountName"  | Set-ADUser -DisplayName "${userFirstName} ${newLastNameIn}" 
}catch{
$modifyUserResult = $false
$err = "$_"
}
...
try{
Remove-ADUser -Identity $TheUserDistinguisedName -Confirm:$False 
}catch{
...
# Modify the user
try{
Set-ADUser -Identity $samAccountName -AccountExpirationDate "$ExpirationDateIn"
}catch{
$modifyUserResult = $false
$err = "$_"
}
